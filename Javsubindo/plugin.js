(function () {
    const BASE_URL = "https://javsubindo.life";
    const HEADERS = {
    ...
    function parseJavsubindoCards(html) {
        const items = [];
        const seen = new Set();
        ...
        while ((match = blockPattern.exec(html)) !== null) {
            ...
            title = title.replace(/\s+-\s+Javsubindo.*$/i, "").replace(/\s+JAVsubid.*$/i, "").trim();
    ...
    async function getHome(cb) {
        try {
            const categories = {
                "Terbaru": `${BASE_URL}/`,
                "Jav Sub Indo": `${BASE_URL}/category/jav-sub-indo/`,
                "4K Videos": `${BASE_URL}/tag/4k/`,
                "Creampie": `${BASE_URL}/tag/creampie/`,
                "Drama": `${BASE_URL}/tag/drama/`,
                "MILF": `${BASE_URL}/tag/milf/`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseJavsubindoCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseJavsubindoCards(html).filter(item => item.url !== url);
