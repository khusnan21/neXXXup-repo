(function () {
    ...
    function parseJavdoeCards(html) {
        const items = [];
        const seen = new Set();
        ...
        while ((match = blockPattern.exec(html)) !== null) {
            const block = match[1];
            if (!block.includes('class="video"')) continue;
    ...
    async function getHome(cb) {
        try {
            const categories = {
                "Latest": `${BASE_URL}/recent/`,
                "New Release": `${BASE_URL}/releaseday/`,
                "English Subtitle": `${BASE_URL}/english-subtitle/`,
                "Asian": `${BASE_URL}/asian/`,
                "Uncensored": `${BASE_URL}/tag/uncensored/`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseJavdoeCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseJavdoeCards(html).filter(item => item.url !== url);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Match onclick attribute value: "onclick=\"playEmbed('...');\""
            const playEmbedRegex = /onclick="[^"]*playEmbed\('([^']+)'\)/gi;
            let match;
            while ((match = playEmbedRegex.exec(html)) !== null) {
            ...
            // Normal button_choice_server class checks
            const serverBtnRegex = /class="[^"]*button_choice_server[^"]*"[^>]*onclick="[^"]*playEmbed\('([^']+)'\)/gi;
