(function () {
    const BASE_URL = "https://javrider.id";
    const HEADERS = {
    ...
    function parseJavriderCards(html) {
        const items = [];
        const seen = new Set();
        
        // Matches article loop-video block or similar
        ...
        while ((match = blockPattern.exec(html)) !== null) {
            ...
            if (!title) {
                const headingM = block.match(/<h[23][^>]*>([\s\S]*?)<\/h[23]>/i) || 
                                 block.match(/class="[^"]*entry-title[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
            ...
            title = title.replace(/\s+-\s+Javrider.*$/i, "").replace(/\s+JavriderId.*$/i, "").trim();

            let poster = "";
            const imgM = block.match(/data-src="([^"]+)"/i) || block.match(/src="([^"]+)"/i) || block.match(/data-original="([^"]+)"/i);
    ...
    async function getHome(cb) {
        try {
            const categories = {
                "Terbaru": `${BASE_URL}/`,
                "Censored Id": `${BASE_URL}/category/censored-id/`,
                "FC2": `${BASE_URL}/category/fc2/`,
                "Subtitle Id": `${BASE_URL}/category/subtitle-id/`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseJavriderCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseJavriderCards(html).filter(item => item.url !== url);
        ...
        try {
            ...
            while ((iframeMatch = iframePattern.exec(html)) !== null) {
                ...
                if (!resolvedEmbeds.has(iframeSrc)) {
                    ...
                    await resolveEmbed(iframeSrc, url, streams);
                }
            }
    ...
    async function resolveEmbed(embedUrl, referer, streams) {
        if (embedUrl.includes("javplayers.com")) {
            await extractJavPlayers(embedUrl, referer, streams);
        } else {
            await loadExtractor(embedUrl, streams);
        }
    }

    async function extractJavPlayers(playerUrl, referer, streams) {
        try {
            const pathParts = playerUrl.split('/');
            let id = pathParts[pathParts.length - 1];
            if (id.includes('?')) id = id.split('?')[0];

            if (!id) return;

            // Simple robust AJAX post execution matching the source
            const apiUrl = "https://javplayers.com/player/index.php?data=" + id + "&do=getVideo";
            const postHeaders = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
            ...
            const postData = `hash=${id}&r=${encodeURIComponent(referer)}`;
            
            // Standard fetch call to bypass post
            ...
            const m3u8 = json.securedLink || json.videoSource;
            if (m3u8) {
                streams.push(new StreamResult({
                    url: m3u8,
                    source: "JavPlayers HLS",
                    headers: { "Referer": "https://javplayers.com/" }
                }));
            }
        } catch (e) {
            console.error("JavPlayers extract failed:", e);
        }
    }
