(function () {
    ...
    function parseJavtifulCards(html) {
        const items = [];
        const seen = new Set();
        const blockPattern = /<article[^>]*class="[^"]*front-video-card[^"]*"[^>]*>([\s\S]*?)<\/article>/gi;
        let match;

        while ((match = blockPattern.exec(html)) !== null) {
            const block = match[1];
            if (block.includes("front-partner-card")) continue;

            const linkM = block.match(/href="([^"]+)"[^>]*class="[^"]*front-video-title[^"]*"/i) || 
                          block.match(/class="[^"]*front-video-title[^"]*"[^>]*href="([^"]+)"/i) ||
                          block.match(/href="([^"]+)"/i);
            if (!linkM) continue;
            let href = linkM[1];
            ...
            // Extract title
            let title = "";
            const titleMatch = block.match(/class="[^"]*front-video-title[^"]*"[^>]*>([\s\S]*?)<\/a>/i);
            ...
            let poster = "";
            const imgM = block.match(/data-front-lazy-src="([^"]+)"/i) || block.match(/src="([^"]+)"/i);
    ...
    async function getHome(cb) {
        try {
            const categories = {
                "Newest": `${BASE_URL}/videos`,
                "Most Viewed": `${BASE_URL}/videos?sort=most_viewed`,
                "Top Rated": `${BASE_URL}/videos?sort=top_rated`,
                "Uncensored": `${BASE_URL}/uncensored`,
                "Chinese AV": `${BASE_URL}/category/chinese-av`,
                "Mature Woman": `${BASE_URL}/category/mature-woman`,
                "Cosplay": `${BASE_URL}/category/cosplay`,
                "Married Woman": `${BASE_URL}/category/married-woman`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseJavtifulCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            let title = "Javtiful Video";
            const titleMatch = html.match(/<div[^>]*class="[^"]*front-watch-title[^"]*"[^>]*>\s*<h1[^>]*>([\s\S]*?)<\/h1>/i) || 
            ...
            const recommendations = parseJavtifulCards(html).filter(item => item.url !== url);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Extract the WatchConfig JSON block:
            // "id=\"frontWatchConfig\" type=\"application/json\">... </script>"
            const configDataMarker = html.indexOf("id=\"frontWatchConfig\"");
            if (configDataMarker !== -1) {
                const startMarker = html.indexOf(">", configDataMarker);
                const endMarker = html.indexOf("</script>", startMarker);
                if (startMarker !== -1 && endMarker !== -1) {
                    const rawJson = html.substring(startMarker + 1, endMarker).trim();
                    try {
                        const parsed = JSON.parse(rawJson);
                        if (parsed && parsed.playerSources && Array.isArray(parsed.playerSources)) {
                            parsed.playerSources.forEach(source => {
                                if (source.src) {
                                    let filename = "Javtiful Stream";
                                    if (source.size) filename += ` (${source.size}p)`;
                        ...
                        console.error("Config JSON parsing error:", err);
                    }
                }
            }

            // Fallback direct iframe scraper
