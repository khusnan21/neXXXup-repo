(function () {
    ...
    function parseJavHDCards(html) {
        const items = [];
        const seen = new Set();
        // Match div containing class vidoes or video
        const blockPattern = /<div[^>]*class="[^"]*(?:video|item|post)[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
        let match;
        ...
        while ((match = blockPattern.exec(html)) !== null) {
            ...
            const titleM = block.match(/class="[^"]*video-title[^"]*"[^>]*>([\s\S]*?)<\/div>/i) || 
                           block.match(/title="([^"]+)"/i) || 
                           block.match(/alt="([^"]+)"/i);
            let title = titleM ? titleM[1].replace(/<[^>]+>/g, '').trim() : "";
            if (!title || title.toLowerCase().includes("actor") || title.toLowerCase().includes("seller")) continue;
        ...
        if (items.length === 0) {
            const fallbackPattern = /<a[^>]*class="[^"]*thumbnail[^"]*"[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;
    ...
    async function getHome(cb) {
        try {
            const categories = {
                "Recent": `${BASE_URL}/recent/?ajax=browse_videos&page=1`,
                "New Release": `${BASE_URL}/releaseday/?ajax=browse_videos&page=1`,
                "Popular": `${BASE_URL}/popular/today/?ajax=browse_videos&page=1`,
                "English Subbed": `${BASE_URL}/eng-sub-jav/?ajax=1`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        ...
                        const items = parseJavHDCards(html);
    ...
    async function search(query, cb) {
        try {
            ...
            const url = `${BASE_URL}/search/video/?s=${encoded}&page=1&ajax=1`;
            const res = await http_get(url, HEADERS);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseJavHDCards(html).filter(item => item.url !== url);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Find base64 embeds
            const choicePattern = /data-embed="([^"]+)"/gi;
            let embedMatch;
            while ((embedMatch = choicePattern.exec(html)) !== null) {
                try {
                    const decoded = atob(embedMatch[1].trim());
            ...
            while ((embedMatch = iframePattern.exec(html)) !== null) {
                let src = embedMatch[1];
                if (src.startsWith('//')) src = 'https:' + src;
                if (!resolvedEmbeds.has(src)) {
                    resolvedEmbeds.add(src);
                    await loadExtractor(src, streams);
                }
            }

            // Direct media patterns
            const filePattern = /(https?:)?\/\/[^\s"'`<>\\?#]+?\.(?:m3u8|mp4)(?:\?[^\s"'`<>\\?#]*)?/gi;
                ...
                if (!link.includes('ads') && !link.includes('google') && !link.includes('facebook')) {
                    if (!streams.some(s => s.url === link)) {
                        streams.push(new StreamResult({
                            url: link,
                            source: link.includes('.m3u8') ? "JAVHD HLS" : "JAVHD Direct",
                            headers: { "Referer": url }
                        }));
                    }
                }
