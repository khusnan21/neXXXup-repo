(function () {
    const BASE_URL = "https://javenglish.cc";
    const HEADERS = {
    ...
    function parseJavEnglishCards(html) {
        const items = [];
        const seen = new Set();
        ...
        while ((match = blockPattern.exec(html)) !== null) {
            ...
            const titleM = block.match(/<span[^>]*>([\s\S]*?)<\/span>/i) || 
                           block.match(/title="([^"]+)"/i) || 
                           block.match(/alt="([^"]+)"/i);
    ...
    async function getHome(cb) {
        try {
            const categories = {
                ...
                "Popular": `${BASE_URL}/page/1/?filter=popular`,
                "Most Viewed": `${BASE_URL}/page/1/?filter=most-viewed`,
                "English Subbed": `${BASE_URL}/category/english-subbed-jav/page/1`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseJavEnglishCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseJavEnglishCards(html).filter(item => item.url !== url);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Scan standard iframes and resource buttons
                ...
                if (!link.includes('ads') && !link.includes('google') && !link.includes('facebook')) {
                    if (!streams.some(s => s.url === link)) {
                        streams.push(new StreamResult({
                            url: link,
                            source: link.includes('.m3u8') ? "HLS Stream" : "Video Direct",
                            headers: { "Referer": url }
                        }));
                    }
                }
