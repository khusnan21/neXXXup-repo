(function () {
    const HEADERS = {
        ...
        "Referer": "https://bondagevalley.com/"
    };

    function parseBondageValleyCards(html) {
        const items = [];
        const cardPattern = /<(div|article)[^>]*class="[^"]*(?:item|post|card|thumb-block)[^"]*"[^>]*>([\s\S]*?)<\/\1>/gi;
        let match;
        while ((match = cardPattern.exec(html)) !== null) {
            ...
            if (href.startsWith('/')) href = 'https://bondagevalley.com' + href;
            if (!href.startsWith('https://bondagevalley.com') || href === 'https://bondagevalley.com/') continue;

            let title = "Bondage Valley Video";
            const titleAttr = block.match(/title="([^"]+)"/i) || block.match(/alt="([^"]+)"/i);
            if (titleAttr) {
                title = titleAttr[1];
            } else {
                const hMatch = block.match(/<h[234][^>]*>([\s\S]*?)<\/h[234]>/i);
            ...
            if (poster && poster.startsWith('/')) poster = 'https://bondagevalley.com' + poster;
    ...
    async function getHome(cb) {
        try {
            const baseUrl = "https://bondagevalley.com";
            const categories = {
                "Latest": `${baseUrl}/`,
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseBondageValleyCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseBondageValleyCards(html);
            ...
            for (const link of uniqueMatches) {
                if (!link.includes('ads') && !link.includes('google')) {
                    streams.push(new StreamResult({
                        url: link,
                        source: link.includes('.m3u8') ? "BondageValley HLS" : "BondageValley Video",
                        headers: { "Referer": url }
                    }));
                }
            }

            // Extract Iframes and Sources
