(function () {
    const HEADERS = {
        ...
        "Referer": "https://desisins.club/"
    };

    function parseDesisinsCards(html) {
        const items = [];
        ...
        while ((match = cardPattern.exec(html)) !== null) {
            ...
            if (href.startsWith('/')) href = 'https://desisins.club' + href;

            let title = "Desisins Video";
            ...
            if (poster && poster.startsWith('/')) poster = 'https://desisins.club' + poster;
    ...
    async function getHome(cb) {
        try {
            const baseUrl = "https://desisins.club";
            const categories = {
                "Home": `${baseUrl}`,
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseDesisinsCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseDesisinsCards(html);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Match direct m3u8 inside script blocks or page html:
            ...
            for (const link of matches) {
                if (!link.includes('ads') && !link.includes('google')) {
                    ...
                    if (!streams.some(s => s.url === fullLink)) {
                        streams.push(new StreamResult({
                            url: fullLink,
                            source: fullLink.includes('.m3u8') ? "Desisins HLS Fallback" : "Desisins Video",
                            headers: { "Referer": url }
                        }));
                    }
                }
