(function () {
    const BASE_URL = "https://javfree.sh";
    const HEADERS = {
    ...
    function parseJavFreeCards(html) {
        const items = [];
        const seen = new Set();
    ...
    async function getHome(cb) {
        try {
            ...
            while ((match = sectionPattern.exec(html)) !== null) {
                ...
                const titleM = sectionHtml.match(/<h2[^>]*class="widget-title"[^>]*>([\s\S]*?)<\/h2>/i);
                const title = titleM ? titleM[1].replace(/<[^>]+>/g, '').trim() : "Unnamed Row";
                const items = parseJavFreeCards(sectionHtml);
                if (items.length > 0) {
                    data[title] = items.slice(0, 20);
                }
            }

            if (Object.keys(data).length === 0) {
                const items = parseJavFreeCards(html);
                if (items.length > 0) {
                    data["Featured Movies"] = items;
                }
            }
    ...
    async function search(query, cb) {
        try {
            ...
            const url = `${BASE_URL}/search/movie/${encoded}`;
            const res = await http_get(url, HEADERS);
            if (res.status !== 200) return cb({ success: false, errorCode: "NETWORK_ERROR" });
            const items = parseJavFreeCards(res.body || "");
    ...
    async function load(url, cb) {
        try {
            ...
            const recommendations = parseJavFreeCards(html).filter(item => item.url !== url);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // Find typical iframes or embed options
                ...
                if (!link.includes('ads') && !link.includes('google') && !link.includes('facebook')) {
                    if (!streams.some(s => s.url === link)) {
                        streams.push(new StreamResult({
                            url: link,
                            source: link.includes('.m3u8') ? "HLS Player" : "Direct Link",
                            headers: { "Referer": url }
                        }));
                    }
                }
