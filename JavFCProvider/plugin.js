(function () {
    const BASE_URL = "https://javfc2.xyz";
    const HEADERS = {
    ...
    function parseJavFCCards(html) {
        const items = [];
        const seen = new Set();
        
        // Match div and article blocks that look like cards/videos
        const blockPattern = /<(div|article)[^>]*class="[^"]*(?:movie-item|video-item|item|column|card)[^"]*"[^>]*>([\s\S]*?)<\/\1>/gi;
        let match;
        
        while ((match = blockPattern.exec(html)) !== null) {
            const block = match[2];
            
            // Check for labels to skip actors or sellers
            const labelM = block.match(/class="[^"]*(?:label|badge)[^"]*"[^>]*>([\s\S]*?)<\/(?:span|div)>/i);
            if (labelM) {
                const text = labelM[1].toLowerCase();
                if (text.includes("actor") || text.includes("seller")) continue;
            }
            ...
            const titleM = block.match(/class="[^"]*movie-title[^"]*"[^>]*>([\s\S]*?)<\/div>/i) || 
                           block.match(/<h[23][^>]*>\s*<a[^>]*>([\s\S]*?)<\/a>/i) ||
                           block.match(/title="([^"]+)"/i) || 
                           block.match(/alt="([^"]+)"/i);
            ...
            let poster = "";
            const posterM = block.match(/data-src="([^"]+)"/i) || 
        ...
        if (items.length === 0) {
            ...
            while ((match = loosePattern.exec(html)) !== null) {
                ...
                if (href.includes('/genre/') || href.includes('/star/') || href.includes('/tag/') || href.includes('vids.html')) continue;
                ...
                if (!title) {
                    ...
                    if (cleanHref) title = cleanHref.replace(/-/g, ' ').replace('.html', '');
                }
    ...
    async function getHome(cb) {
        try {
            const categories = {
                "Terbaru": `${BASE_URL}/home/vids.html`,
                "All Movies": `${BASE_URL}/all-movies.html`,
                "Ranking": `${BASE_URL}/home/ranking.html`,
                "Engsub": `${BASE_URL}/genre/eng-sub.html`,
                "FC2PPV": `${BASE_URL}/genre/fc2.html`,
                "JAV": `${BASE_URL}/genre/jav.html`
            };
            ...
            for (const [name, url] of Object.entries(categories)) {
                try {
                    ...
                    if (res.status === 200 && res.body) {
                        const items = parseJavFCCards(res.body);
    ...
    async function load(url, cb) {
        try {
            ...
            let title = "JavFC Video";
            const titleMatch = html.match(/<h1[^>]*class="[^"]*(?:title|video-title)[^"]*"[^>]*>([\s\S]*?)<\/h1>/i) || 
            ...
            const recommendations = parseJavFCCards(html).filter(item => item.url !== url);
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            // player-div scripts or embed patterns
            ...
            // Direct match and generic loader
                ...
                if (!link.includes('ads') && !link.includes('google') && !link.includes('facebook')) {
                    if (!streams.some(s => s.url === link)) {
                        streams.push(new StreamResult({
                            url: link,
                            source: link.includes('.m3u8') ? "HLS Streaming" : "Direct Video",
                            headers: { "Referer": url }
                        }));
                    }
                }
